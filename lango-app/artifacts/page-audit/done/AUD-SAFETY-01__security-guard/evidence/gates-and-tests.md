...
| ESLint touched | PASS — new files clean; `gates-service` back to its baseline 30 style errors; `kiosk-service`/`credential-adapter`/`visitors-service`/`home-service` counts unchanged; `today.ts` clean |
...
6. guard home is branch-scoped (own + tenant-wide invitations only) and still
   returns the assignment's gate/shift;
7. emergency activation is leadership-only (guard 403, admin 201).

Related suites re-run (all pass):

```
release-live-link, onsite-headcount, guardians-domain, security,
role-response-shape, student-360-hardening → 6 files, 77/77 PASS
```

Combined focused total: **84/84**.
