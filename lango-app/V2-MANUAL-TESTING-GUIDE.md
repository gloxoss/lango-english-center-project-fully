...
- **Lango Center Admin**: `admin@lango.ma` / `Admin123!` (Role: `school_admin`, Tenant: Lango English Center)
...
4. In Browser Window B (or Incognito), log in as `admin@lango.ma` (Lango Center Admin).
5. Go to **Élèves > Repertoire** (`/fr/dashboard/students`).
6. **Expected result**: None of the students from Groupe Scolaire Atlas are visible in Lango's directory. Each tenant sees exclusively its own data.
...
1. Log in as `admin@lango.ma` / `Admin123!`.
...
2. Log in as `admin@lango.ma`, go to **Présence > Justificatifs** (`/fr/dashboard/attendance/excuses`).
...
2. Log in as `admin@lango.ma`, go to **Présence > Audit & Alertes**.
...
1. Log in as `admin@lango.ma`, open any student's profile (`/fr/dashboard/students/<id>`).
...
2. **Expected result**: Atlas's flags, excuses, and audit KPIs are entirely independent of Lango's — no cross-tenant leakage on `/dashboard/attendance/excuses`, `/dashboard/attendance/audit`, or `GET /api/attendance/flags`.
