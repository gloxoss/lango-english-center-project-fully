...
5. **`liveClassParticipantEvents`** — sessionId FK (cascade), `providerEventId` (immutable), `providerProfileId` FK, userId? (resolved SchoolOS identity), externalParticipantId?, participantRole?, `eventType` (pgEnum: joined/left/reconnect/error/kicked/muted/consent_accepted/recording_started/recording_stopped), `providerTimestamp`, `receivedTimestamp`, `rawPayload jsonb` (bounded diagnostic evidence only), `processingStatus`, `retries`. **Unique `(tenantId, providerEventId)`** = webhook/duplicate dedupe.
