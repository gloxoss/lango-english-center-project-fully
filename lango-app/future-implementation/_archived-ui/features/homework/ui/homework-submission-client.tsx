...
export function HomeworkSubmissionClient({ locale }: { locale?: string } = {}) {
  ...
  const handleRequestRevision = () => {
    ...
    setSubmissions(prev => prev.map(s => s.id === selectedSub.id ? {
      ...
      feedback: inputFeedback || t('requestRevisionAction'),
    } : s));
  };
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-[#16212B] tracking-tight">{t('submissionEvaluationTitle')}</h1>
          <p className="text-xs text-slate-500 mt-1">{t('submissionEvaluationSubtitle')}</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-5 space-y-3">
          <Card className="p-3 bg-white rounded-2xl border border-slate-200/80 shadow-2xs font-extrabold text-xs text-[#16212B] flex justify-between items-center">
            <span>{t('receivedCopies', { count: submissions.length })}</span>
            <span className="text-[10px] text-slate-400 font-normal">{t('allClassesCombined')}</span>
          </Card>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400">{sub.matricule} • {t('submittedOnDate', { date: sub.submittedAt })}</p>
                    </div>
                  </div>
                  ...
                  <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full ${
                    ...
                    {sub.status === 'Graded' ? `${sub.score}/${sub.maxScore}` : sub.status === 'Needs Revision' ? t('needsRevisionBadge') : t('pendingBadge')}
                  </span>
                </div>
              </Card>
            );
            ...
            <Card className="p-6 bg-white rounded-2xl border border-slate-200/80 shadow-2xs space-y-5">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-base font-extrabold text-[#16212B]">{t('studentPaper', { name: selectedSub.studentName })}</h2>
                    <p className="text-xs text-slate-400">{t('fileLabel', { name: selectedSub.fileName, size: selectedSub.fileSize })}</p>
                  </div>
                </div>
              ...
              >
                ...
                <p className="text-xs font-bold">{t('digitalViewerTitle')}</p>
                <p className="text-[10px] text-slate-400">{t('digitalViewerDesc')}</p>
              </div>
              ...
              <div className="space-y-4 pt-2">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-bold text-slate-700 block mb-1">{t('assignedGrade')}</label>
                    <Input
                      type="number"
                      step="0.5"
                      placeholder="Ex: 18.5"
                      ...
                      className="h-10 text-sm font-extrabold rounded-xl text-start"
                    />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-slate-700 block mb-1">{t('generalFeedback')}</label>
                    <Input
                      placeholder={t('feedbackPlaceholder')}
                      value={inputFeedback}
                      onChange={e => setInputFeedback(e.target.value)}
                      className="h-10 text-xs rounded-xl text-start"
                    />
                  </div>
                </div>
          ...
          <DialogHeader>
            <DialogTitle className="text-base font-extrabold text-white flex items-center justify-between">
              <span className="flex items-center gap-2">
                ...
                {t('pdfCorrectionTitle', { fileName: selectedSub?.fileName ?? '' })}
              </span>
            </DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-4 my-4">
              <div className="bg-slate-950 rounded-2xl h-[450px] border border-slate-800 flex flex-col items-center justify-center space-y-3 p-6 text-center">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">{t('studentInfo', { name: selectedSub.studentName, matricule: selectedSub.matricule })}</p>
                </div>
