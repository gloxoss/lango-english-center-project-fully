...
**Totals:** 31 IMPLEMENTED · 3 PARTIAL · 1 IN PROGRESS · 1 NOT STARTED
...
- [ ] **Stale plan docs (10 plans):** self-report "not started/planned" while code is built — `custom-domain`, `inventory`, `hostel`, `guard`, `transport`, `library`, `alumni`, `advanced-reporting`, `dropped-features-rebuild`, `assessment`. Reverse drift: `advanced-reporting` + `assessment` EXECUTION-AUDIT docs claim "100% deployed" but were disproven. Refresh doc status lines as you touch each plan.
...
- **Found in code:** migration `0060` (22 tables); `assessment-schema.ts`; services `outcome/homework/exam-master/online-exam`; Exam Master fully wired (`exam-terms/halls/schedules`, seat-allocation/marksheet/rankings); remediation applied to `online-exams/submit` + `homework-service`; 15 tests.
- **Gaps / next actions:**
  - [ ] **Decide the online-exams addon fate.** `OnlineExamService` + `online_exam_attempts/responses` schema are NOT wired to any route (table-name collision with legacy migration `0025`). Either wire them properly or retire the duplicate.
  - [ ] Server-timed runner, live monitor, manual-grading tasks, accommodations.
  - [ ] Question groups/blueprints, QTI export/import.
  - [ ] Update stale STATE.md ("Phase 0 IN_PROGRESS") + EXECUTION-AUDIT-REPORT ("100% deployed" — disproven).
- **Key files:** `src/features/assessment/`, `src/app/api/academics/online-exams/submit/route.ts`, `migrations/0060`.
...
- **Gaps / next actions:**
  - [ ] `payroll-posting.ts` — explicitly Payroll-owned, unbuilt (blocked by #16).
  - [ ] Morocco tax/statutory mappings uncertified (explicit non-certification).
...
4. **Finance correctness** — start student-accounting (#12); finish online-exams addon decision (#10); payroll posting + tax mappings (#16/#13).
