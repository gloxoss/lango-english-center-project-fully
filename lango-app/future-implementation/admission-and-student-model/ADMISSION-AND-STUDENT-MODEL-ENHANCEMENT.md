...
| **Nationality** | More relevant than the reference form's own fields for an "international" school context (see `subscription-licensing/`'s stated ambition) — Lango likely serves both Moroccan and expat families; nationality is a normal, non-sensitive administrative field school systems legitimately track, unlike the excluded fields below. |
...
| **Transport Details / Hostel Details on the admission form** | Before building either: confirm these are actually relevant to this school's business model at all. Lango is a language center — check whether it operates any bus routes or dormitories before speccing an admission-form section for services that may not exist. (Transport and Hostel are already tracked as possible future addons in `src/addons/registry.ts` if genuinely wanted — but that's a separate decision from whether the *admission form* needs a section for them.) |
