...
export function ResidentMeView() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#16212B]">{t('residentPortalTitle')}</h1>
        <p className="text-sm text-slate-500">{t('residentPortalSubtitle')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-10 text-center text-sm text-slate-500">{t('residentLoadError')}</div>
      ) : !enrolled ? (
        <div className="rounded-2xl border border-slate-200/80 bg-white p-10 text-center">
          ...
          <p className="font-semibold text-[#16212B]">{t('notAccommodatedTitle')}</p>
          <p className="mt-1 text-sm text-slate-500">{t('notEnrolledHostel')}</p>
        </div>
      ) : (
        <>
          <Card className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-2xs">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <Button onClick={() => setRequestOpen(true)}><Plus className="mr-2 h-4 w-4" /> {t('requestLeavePass')}</Button>
            </div>
            {data.tonight && (
              <div className="mt-4 grid gap-3 border-t border-slate-100 pt-4 sm:grid-cols-3">
                <div className="flex items-center gap-2 rounded-xl bg-slate-50 px-3 py-2">
                  ...
                  <div>
                    <p className="text-xs text-slate-500">{t('tonightRollCall')}</p>
                    <p className="text-sm font-semibold text-[#16212B]">{data.tonight.rollCallStatus ? ROLL_CALL_LABELS[data.tonight.rollCallStatus] ?? data.tonight.rollCallStatus : t('notCheckedYet')}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 rounded-xl bg-slate-50 px-3 py-2">
                  <DoorOpen className="h-4 w-4 text-slate-400" />
                  <div>
                    <p className="text-xs text-slate-500">{t('leaveTonight')}</p>
                    <p className="text-sm font-semibold text-[#16212B]">{data.tonight.onLeaveTonight ? t('authorized') : t('notAuthorized')}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 rounded-xl bg-slate-50 px-3 py-2">
                  <UserX className="h-4 w-4 text-slate-400" />
                  <div>
                    <p className="text-xs text-slate-500">{t('expectedReturn')}</p>
                    <p className="text-sm font-semibold text-[#16212B]">
                      {data.tonight.leavePass ? new Date(data.tonight.leavePass.expectedReturnAt).toLocaleString() : '—'}
                      {data.tonight.overdueReturn ? ` ${t('overdueSuffix')}` : ''}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <Card className="rounded-2xl border border-slate-200/80 bg-white shadow-2xs">
            <div className="border-b border-slate-100 p-4">
              <h2 className="font-semibold text-[#16212B]">{t('myLeavePassesCount', { count: data.leavePasses.length })}</h2>
            </div>
            <div className="divide-y divide-slate-100">
              {data.leavePasses.length === 0 && (
                <div className="p-6 text-center text-sm text-slate-500">{t('noLeavePasses')}</div>
              )}
              {data.leavePasses.map(p => (
                <div key={p.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-[#16212B]">
                        {new Date(p.startDateTime).toLocaleString()} → {new Date(p.expectedReturnAt).toLocaleString()}
                      </p>
      ...
      <Dialog open={requestOpen} onOpenChange={setRequestOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{t('requestLeaveDialogTitle')}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="mb-1 block text-sm font-medium text-slate-700">{t('departureDate')}</label>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setRequestOpen(false)}>{t('btnCancel')}</Button>
            <Button onClick={requestLeave} disabled={saving || !form.startDateTime || !form.expectedReturnAt}>
              {saving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} {t('btnSubmit')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
