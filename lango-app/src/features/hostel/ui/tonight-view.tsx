...
type TonightData = {
  ...
  overdueCheckouts?: { allocationId: string; studentName: string | null; roomCode: string; bedCode: string; effectiveEndDate: string }[];
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      {data && (
        <>
          ...
          {(data.overdueCheckouts?.length ?? 0) > 0 && (
            <Card className="rounded-2xl border border-amber-300 bg-amber-50 p-5 shadow-2xs" role="status">
              <h2 className="font-semibold text-amber-900">{t('overdueCheckoutsTitle', { count: data.overdueCheckouts!.length })}</h2>
              <p className="mt-1 text-sm text-amber-800">{t('overdueCheckoutsHint')}</p>
              <ul className="mt-3 divide-y divide-amber-200 text-sm">
                {data.overdueCheckouts!.map(o => (
                  <li key={o.allocationId} className="flex flex-wrap items-center justify-between gap-2 py-2">
                    <span className="font-medium text-amber-950">{o.studentName ?? '—'} · {o.roomCode} / {o.bedCode}</span>
                    <span className="flex items-center gap-3 text-amber-800">
                      {t('overdueCheckoutEnded', { date: o.effectiveEndDate })}
                      <Link href={`/dashboard/hostel/allocations/${o.allocationId}`} className="font-semibold text-[#2487B8] hover:underline">{tCommon('view')}</Link>
                    </span>
                  </li>
                ))}
              </ul>
