...
export function PeriodsView({ locale: _locale }: { locale?: string } = {}) {
  ...
  const submitCreate = async (e: React.FormEvent) => {
    ...
    setBusy(false);
    if (outcome.ok) {
      setNotice(t('periodCreatedSuccess'));
      setDialog(null);
      setForm({ name: '', startDate: '', endDate: '', reason: '', note: '' });
      load();
    } else {
  ...
  const submitClose = async (e: React.FormEvent) => {
    ...
    setBusy(false);
    if (outcome.ok) {
      setNotice(t('periodClosedSuccess'));
      setDialog(null);
      setTarget(null);
      setForm({ name: '', startDate: '', endDate: '', reason: '', note: '' });
      load();
  ...
  const submitReopen = async (e: React.FormEvent) => {
    ...
    setBusy(false);
    if (outcome.ok) {
      setNotice(t('reopenSubmittedSuccess'));
      setDialog(null);
      setTarget(null);
      setForm({ name: '', startDate: '', endDate: '', reason: '', note: '' });
      load();
  ...
  const decide = async (request: ReopenRequest, decision: 'approved' | 'rejected') => {
    ...
    setBusy(false);
    if (outcome.ok) {
      setNotice(decision === 'approved' ? t('reopenApprovedSuccess') : t('reopenRejectedSuccess'));
      setForm(f => ({ ...f, note: '' }));
      load();
    } else {
      setError(outcome.message ?? t('actionFailed'));
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between text-start">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">{t('fiscalPeriodsTitle')}</h1>
          <p className="text-sm text-slate-500">{t('fiscalPeriodsDesc')}</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button onClick={() => openDialog('create')} className="gap-2 bg-[#2487B8] text-white hover:bg-[#1B6C93]">
            <PlusCircle className="size-3.5" />
            {t('newFiscalPeriodBtn')}
          </Button>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-start text-xs">
            <thead className="border-b border-slate-200 bg-slate-50 font-bold uppercase text-slate-500">
              <tr>
                <th className="px-4 py-3 text-start">{t('periodLabel')}</th>
                ...
                <th className="px-4 py-3 text-start">{t('reopenRequestBtn') || t('reopenSubmittedSuccess')}</th>
                    ...
                    <tr className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        ...
                        >
                          ...
                          {p.status === 'open' ? tCommon('active') : tCommon('closed') || 'Clôturée'}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <button
                          onClick={() => setShowRequests(s => ({ ...s, [p.id]: !s[p.id] }))}
                          className="font-bold text-[#0066FF] hover:underline"
                        >
                          {t('periodReopenRequestsTitle', { count: queue.length })}
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          {p.status === 'closed' && (
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => openDialog('reopen', p)}
                              className="text-[#2487B8] border-slate-200 hover:bg-slate-50 text-xs"
                            >
                              {t('requestReopenAction')}
                            </Button>
                          )}
                        </div>
                      </td>
                    ...
                    {showRequests[p.id] && (
                      <tr>
                        <td colSpan={6} className="bg-slate-50/70 px-4 py-3">
                          {queue.length === 0 ? (
                            <span className="text-xs text-slate-500">{t('noReopenRequests')}</span>
                          ) : (
                                ...
                                >
                                  <div className="min-w-0 text-start">
                                    ...
                                    <p className="text-[11px] text-slate-500">
                                      {fmt(q.createdAt)} · {tCommon('status')} : {q.status}
                                      {q.decidedAt ? ` · ${fmt(q.decidedAt)}` : ''}
                                  ...
                                  {q.status === 'pending' && (
                                    <div className="flex items-center gap-2">
                                      <Input
                                        placeholder={t('decisionNoteLabel')}
                                        value={form.note}
      ...
      {dialog === 'create' && (
        <Modal title={t('newFiscalPeriodBtn')} onClose={() => setDialog(null)}>
          <form onSubmit={submitCreate} className="space-y-4 text-start">
            ...
            <ModalActions busy={busy} label={t('newFiscalPeriodBtn')} busyLabel={tCommon('saving')} />
          </form>
        </Modal>
      )}

      {dialog === 'close' && target && (
        <Modal title={`${t('closePeriodAction')} — ${target.name}`} onClose={() => setDialog(null)}>
          <form onSubmit={submitClose} className="space-y-4 text-start">
            <p className="text-xs text-slate-500">
              {t('fiscalPeriodsDesc')}
            </p>
            <Field label={t('periodClosingReason')}>
              <Input
                value={form.reason}
                onChange={e => setForm(f => ({ ...f, reason: e.target.value }))}
                placeholder={t('periodClosingReason')}
                required
                minLength={3}
              />
            </Field>
            <ModalActions busy={busy} label={t('closePeriodAction')} busyLabel={tCommon('saving')} />
          </form>
        </Modal>
      )}

      {dialog === 'reopen' && target && (
        <Modal title={`${t('requestReopenAction')} — ${target.name}`} onClose={() => setDialog(null)}>
          <form onSubmit={submitReopen} className="space-y-4 text-start">
            <p className="text-xs text-slate-500">
              {t('fiscalPeriodsDesc')}
            </p>
            <Field label={t('periodReopenReason')}>
              <Input
                value={form.reason}
                onChange={e => setForm(f => ({ ...f, reason: e.target.value }))}
                placeholder={t('periodReopenReason')}
                required
                minLength={10}
              />
            </Field>
            <ModalActions busy={busy} label={t('requestReopenAction')} busyLabel={tCommon('saving')} />
          </form>
        </Modal>
      )}
    </div>
...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-xs">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl text-start">
...
function ModalActions({ busy, label, busyLabel }: { busy: boolean; label: string; busyLabel: string }) {
  return (
    <Button type="submit" disabled={busy} className="w-full bg-[#2487B8] text-white hover:bg-[#1B6C93]">
