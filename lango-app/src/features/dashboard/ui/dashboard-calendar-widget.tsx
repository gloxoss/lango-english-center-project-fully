...
export function DashboardCalendarWidget({
  ...
  const weekdays = Array.from({ length: 7 }, (_, i) => weekdayFormatter.format(new Date(2026, 0, 5 + i)));
    ...
    <Card className="p-5 bg-white rounded-2xl border border-slate-200/80 shadow-2xs flex flex-col justify-between h-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-100 pb-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 font-medium">{t('calendarSubtitle')}</p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-4 pt-3 border-t border-slate-100 text-[11px]">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-slate-600 font-medium">{t('calendarVacationLegend')}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full bg-amber-400" />
          <span className="text-slate-600 font-medium">{t('calendarHolidayLegend')}</span>
        </div>
      </div>
    </Card>
  );
