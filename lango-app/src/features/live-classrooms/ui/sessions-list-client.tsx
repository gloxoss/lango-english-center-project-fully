...
export function SessionsListClient({ locale }: { locale: string }) {
  ...
  return (
    <div className="space-y-6 max-w-[1800px] mx-auto">
      ...
      {/* Context Banner: Presential vs Remote */}
      <div className="rounded-2xl border border-sky-100 bg-sky-50/70 p-4 text-start flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-2xs">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-sky-100 text-[#2487B8] flex items-center justify-center shrink-0">
            <MonitorPlay className="w-5 h-5" />
          </div>
          <div>
            <h3 className="text-xs font-bold text-[#16212B]">Usage du Module : Cours à Distance & Soutien Scolaire</h3>
            <p className="text-[11px] text-slate-600 mt-0.5">
              Ce module est dédié aux séances virtuelles et au tutorat du soir avec caméra et prise de présence automatique. Si votre établissement opère à 100% en présentiel, vous pouvez ignorer ce module sans aucun impact sur votre fonctionnement scolaire.
            </p>
          </div>
        </div>
        <Link
          href={`/${locale}/dashboard/academics/live-class-reports`}
          className="inline-flex items-center gap-1 text-xs font-bold text-[#2487B8] hover:underline shrink-0"
        >
          Rapports de présence &rarr;
        </Link>
      </div>
