...
export function IssueCertificateDialog(props: Props) {
  ...
  const handleIssue = async () => {
    ...
    try {
      ...
      if (!json.success) {
        setError(json.message || json.error?.message || t('errorIssueCert'));
        return;
      }
      setResult(json.data);
    } catch {
    ...
    <Dialog open={open} onOpenChange={(o) => { if (!issuing) onOpenChange(o); }}>
      <DialogContent className="sm:max-w-[520px]">
        <DialogHeader className="text-start">
          <DialogTitle>{t('dialogIssueCertTitle')}</DialogTitle>
          <DialogDescription>
        ...
        {result ? (
          <div className="py-4 space-y-4 text-start">
            <div className="rounded-2xl border border-emerald-200 bg-[#DDF5EC] p-4">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-extrabold text-[#17A673]">{t('certIssuedSuccess')}</p>
              </div>
              ...
              <div className="mt-3 flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" className="h-8 text-xs cursor-pointer" onClick={copyToken}>
                  {copied ? <CheckCircle2 className="w-3.5 h-3.5 text-[#17A673] me-1.5" /> : <Copy className="w-3.5 h-3.5 me-1.5" />}
            ...
            <div className="flex flex-col gap-2">
              {result.pdfBase64 ? (
                ...
                >
                  <Download className="w-4 h-4 me-1.5" />{t('btnDownloadPdf')}
                </a>
              ) : (
                <p className="text-[11px] font-semibold text-amber-600 text-center">
                  {t('pdfNotAutoGenerated')}
                </p>
              )}
              <a
                href={`/${locale}/verify/certificate/${result.rawToken}`}
                target="_blank"
              ...
              >
                <ExternalLink className="w-4 h-4 me-1.5" />{t('btnTestVerificationPage')}
              </a>
            </div>
          </div>
        ) : (
          <div className="py-4 space-y-4 text-start">
            <div className="space-y-2">
              <Label className="text-xs font-bold text-slate-700">{t('labelCertDefinition')}</Label>
              <Select value={selectedDefinitionId} onValueChange={selectDefinition}>
                ...
                <SelectContent>
                  {definitions.length === 0 ? (
                    <p className="px-3 py-2 text-xs text-slate-400">{t('noDefinitionForType')}</p>
                  ) : (
                    definitions.map(d => (
          ...
          {result ? (
            ...
            <>
              ...
              >
                ...
                {issuing ? t('btnIssuing') : t('btnIssueCert')}
              </Button>
            </>
          )}
        </DialogFooter>
