...
const STAGES_META: { key: ProspectStage; color: string; bg: string }[] = [
  { key: 'new', color: 'text-[#0066FF]', bg: 'bg-blue-50 border-blue-200' },
  { key: 'contacted', color: 'text-amber-700', bg: 'bg-amber-50 border-amber-200' },
  { key: 'qualified', color: 'text-purple-700', bg: 'bg-purple-50 border-purple-200' },
  { key: 'converted', color: 'text-emerald-700', bg: 'bg-emerald-50 border-emerald-200' },
  { key: 'lost', color: 'text-rose-700', bg: 'bg-rose-50 border-rose-200' },
];
...
export function InquiriesKanbanView() {
  ...
  const getStageLabel = useCallback((stg: ProspectStage) => {
    switch (stg) {
      case 'new': return t('stageNew');
      case 'contacted': return t('stageContacted');
      case 'qualified': return t('stageQualified');
      case 'converted': return t('stageConverted');
      case 'lost': return t('stageLost');
      default: return stg;
    }
  }, [t]);

  const getSourceLabel = useCallback((src: InquirySource | 'all') => {
    switch (src) {
      case 'all': return t('allSources');
      ...
      case 'facebook_ads': return t('sourceFacebookAds');
      case 'google_ads': return t('sourceGoogleAds');
      default: return src;
    }
  }, [t]);

  const getInterestLabel = useCallback((lvl: InterestLevel) => {
    switch (lvl) {
  ...
  const sourcesList: { value: InquirySource | 'all'; label: string }[] = useMemo(() => [
    { value: 'all', label: getSourceLabel('all') },
    { value: 'web', label: getSourceLabel('web') },
    { value: 'phone', label: getSourceLabel('phone') },
    { value: 'walk_in', label: getSourceLabel('walk_in') },
    { value: 'referral', label: getSourceLabel('referral') },
    { value: 'facebook_ads', label: getSourceLabel('facebook_ads') },
    { value: 'google_ads', label: getSourceLabel('google_ads') },
  ], [getSourceLabel]);
  ...
  const loadPipeline = useCallback(async () => {
    ...
    try {
      ...
      if (ownerFilter) q.set('assignedToId', ownerFilter);

      const res = await api(`/api/crm/inquiries?${q.toString()}`);
      setInquiries(res.data?.rows ?? []);
      setTotal(res.data?.total ?? 0);

      const countsMap: Record<ProspectStage, number> = { new: 0, contacted: 0, qualified: 0, converted: 0, lost: 0 };
      for (const row of res.data?.rows ?? []) {
        const s = row.status as ProspectStage;
        if (countsMap[s] !== undefined) countsMap[s]++;
      }
      setCounts(countsMap);
    } catch (e) {
  ...
  }, [page, search, source, tag, ownerFilter, tCommon]);

  // Load staff list once
  useEffect(() => {
    api('/api/hr/employees?status=active')
      .then((res) => {
        const items: StaffMember[] = (res.data?.items ?? res.data ?? []).map((e: any) => ({
          id: e.userId ?? e.id,
          fullName: e.fullName ?? `${e.firstName ?? ''} ${e.lastName ?? ''}`.trim() ?? e.name,
          role: e.designation ?? e.jobTitle ?? 'Staff',
        }));
        setStaff(items.filter((x) => x.id && x.fullName));
      })
  ...
  // Stage transition
  const updateStage = async (id: string, newStage: ProspectStage) => {
    setBusyAction('stage');
    try {
      await api(`/api/crm/inquiries/${id}/stage`, {
        method: 'POST',
        body: JSON.stringify({ stage: newStage }),
      });
      showToast('ok', t('leadUpdatedToast'));
      await loadPipeline();
      if (selected?.id === id) {
        setSelected((prev) => (prev ? { ...prev, status: newStage } : prev));
      }
    } catch (e) {
      showToast('err', e instanceof Error ? e.message : tCommon('error'));
    } finally {
      setBusyAction(null);
    }
  };

  const handleDrop = async (targetStage: ProspectStage) => {
    if (!dragId) return;
    const current = inquiries.find((x) => x.id === dragId);
    if (!current || current.status === targetStage) return;
    setDragOverStage(null);
    await updateStage(dragId, targetStage);
    setDragId(null);
  };
  ...
  const openProfile = async (id: string) => {
    ...
    try {
      const res = await api(`/api/crm/inquiries/${id}`);
  ...
  const toggleTag = (tName: string) => {
    if (!selected) return;
    const tags = selected.tags ?? [];
    const next = tags.includes(tName) ? tags.filter((x) => x !== tName) : [...tags, tName];
  ...
  const addFollowUp = async (type: FollowUp['type'], notes: string) => {
    ...
    try {
      ...
      showToast('ok', t('followUpSavedToast'));
    } catch (e) {
      showToast('err', e instanceof Error ? e.message : tCommon('error'));
    } finally {
      setBusyAction(null);
  ...
  const convert = async (id: string) => {
    ...
    try {
      ...
      showToast('ok', t('leadConvertedToast'));
      await loadPipeline();
  ...
  const removeLead = async () => {
    ...
    try {
      ...
      showToast('ok', t('leadDeletedToast'));
      setSelected(null);
      await loadPipeline();
    } catch (e) {
      showToast('err', e instanceof Error ? e.message : tCommon('error'));
  ...
  const createLead = async (e: React.FormEvent) => {
    ...
    setCreating(true);
    try {
      const tags = newLead.tags.split(',').map((item) => item.trim()).filter(Boolean);
      ...
      showToast('ok', t('leadCreatedToast'));
      setPage(1);
      await loadPipeline();
    } catch (err) {
      showToast('err', err instanceof Error ? err.message : tCommon('error'));
    } finally {
      setCreating(false);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto pb-12">
      {toast && (
        <div className={`fixed top-4 end-4 z-[70] px-4 py-3 rounded-xl shadow-lg border text-sm font-bold ${toast.kind === 'ok' ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-rose-50 border-rose-200 text-rose-800'}`}>
          {toast.text}
        </div>
      )}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-xs font-extrabold text-[#16212B] flex items-center gap-2">
              {t('capacityGauge')}:
              <Badge variant="info" className="text-[10px] font-bold">{total} / 1000 {t('capacityUsed')}</Badge>
            </span>
            <p className="text-[11px] text-slate-500 font-medium mt-0.5">{t('capacityVolume')}</p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-1.5">
          {STAGES_META.map((stg) => (
            <Badge key={stg.key} variant="neutral" className={`text-[10px] font-bold ${stg.bg} ${stg.color}`}>
              {getStageLabel(stg.key)}: {counts[stg.key] ?? 0}
            </Badge>
          ))}
        </div>
      </Card>
      ...
      <div className="flex flex-col gap-4 bg-white p-4 rounded-2xl border border-slate-200/80 shadow-2xs lg:flex-row lg:items-center lg:justify-between">
        <div className="relative flex-1 max-w-md">
          ...
          <Input
            ...
            className="ps-9 text-xs rounded-xl border-slate-200 bg-slate-50/50 focus:bg-white h-10 font-medium text-slate-800"
          />
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative">
            <Filter className="absolute start-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" />
            <select
              value={source}
              onChange={(e) => setSource(e.target.value as any)}
              className="ps-9 pe-3 h-10 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-700 cursor-pointer"
            >
              {sourcesList.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
            </select>
          </div>
          <div className="relative">
            <TagIcon className="absolute start-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" />
            <Input value={tag} onChange={(e) => setTag(e.target.value)} placeholder={t('tags')} className="ps-9 text-xs rounded-xl border-slate-200 h-10 w-28" />
          </div>
          {!staffError && (
            <select value={ownerFilter} onChange={(e) => setOwnerFilter(e.target.value)} className="h-10 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-700 px-3 cursor-pointer">
            ...
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between pb-2.5 border-b border-slate-200/80">
                  <span className={`text-xs font-extrabold px-3 py-1 rounded-xl border ${stg.bg} ${stg.color}`}>{getStageLabel(stg.key)}</span>
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <Badge variant={item.interestLevel === 'high' ? 'success' : item.interestLevel === 'medium' ? 'info' : 'neutral'} className="text-[9px] font-extrabold uppercase px-1.5 py-0.5 shrink-0">
                          {getInterestLabel(item.interestLevel)}
                        </Badge>
                      </div>
                      <div className="text-[11px] text-slate-500 space-y-1">
                        {item.phone && <p className="flex items-center gap-1.5 font-medium"><Phone className="w-3 h-3 text-[#0066FF] shrink-0" /><span>{item.phone}</span></p>}
                        {item.email && <p className="flex items-center gap-1.5 font-medium truncate"><Mail className="w-3 h-3 text-slate-400 shrink-0" /><span className="truncate">{item.email}</span></p>}
                      </div>
                      {(item.tags ?? []).length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {item.tags!.slice(0, 3).map((tTag) => <Badge key={tTag} variant="neutral" className="text-[9px] font-bold text-[#0066FF]">{tTag}</Badge>)}
                        </div>
                      )}
                      ...
                      <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[10px] font-semibold text-slate-400">
                        <span className="uppercase text-[#0066FF] font-bold">{getSourceLabel(item.source)}</span>
                        <span>{new Date(item.createdAt).toLocaleDateString(dateLocale)}</span>
                      </div>
                    </Card>
                  ))}
                  {colItems.length === 0 && (
                    <div className="p-4 text-center text-xs text-slate-400 font-medium py-12 border border-dashed border-slate-200 rounded-xl">{t('noLeadsInStage')}</div>
                  )}
                </div>
              </div>
            </div>
      ...
      <div className="flex items-center justify-between">
        <span className="text-xs font-bold text-slate-500">{total} {t('totalProspects')} — page {page}/{totalPages}</span>
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" disabled={page <= 1 || loading} onClick={() => setPage((p) => p - 1)} className="rounded-xl text-xs cursor-pointer">{tCommon('previous')}</Button>
          <Button variant="outline" size="sm" disabled={page >= totalPages || loading} onClick={() => setPage((p) => p + 1)} className="rounded-xl text-xs cursor-pointer">{tCommon('next')}</Button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-md w-full p-6 space-y-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h2 className="text-base font-extrabold text-[#16212B]">{t('btnAddLead')}</h2>
            ...
            <form onSubmit={createLead} className="space-y-3.5">
              <div>
                ...
                <Input type="text" required value={newLead.contactName} onChange={(e) => setNewLead({ ...newLead, contactName: e.target.value })} className="mt-1 text-xs rounded-xl" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-slate-700">{t('phone')}</label>
                  <Input type="text" value={newLead.phone} onChange={(e) => setNewLead({ ...newLead, phone: e.target.value })} className="mt-1 text-xs rounded-xl" />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-700">{t('email')}</label>
                  <Input type="email" value={newLead.email} onChange={(e) => setNewLead({ ...newLead, email: e.target.value })} className="mt-1 text-xs rounded-xl" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-slate-700">{t('filterSource')}</label>
                  <select value={newLead.source} onChange={(e) => setNewLead({ ...newLead, source: e.target.value as any })} className="mt-1 w-full p-2.5 text-xs rounded-xl border border-slate-200 font-medium">
                    {sourcesList.filter((s) => s.value !== 'all').map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-700">{t('interestLevel')}</label>
                  <select value={newLead.interestLevel} onChange={(e) => setNewLead({ ...newLead, interestLevel: e.target.value as any })} className="mt-1 w-full p-2.5 text-xs rounded-xl border border-slate-200 font-medium">
                    <option value="low">{t('interestLow')}</option>
                    <option value="medium">{t('interestMedium')}</option>
                    <option value="high">{t('interestHigh')}</option>
                  </select>
                </div>
              </div>
              {!staffError && (
                <div>
                  <label className="text-xs font-bold text-slate-700">{t('assignedStaff')}</label>
              ...
              <div>
                <label className="text-xs font-bold text-slate-700">{t('tags')}</label>
                <Input value={newLead.tags} onChange={(e) => setNewLead({ ...newLead, tags: e.target.value })} placeholder={t('tagsPlaceholder')} className="mt-1 text-xs rounded-xl" />
              </div>
              <div>
                <label className="text-xs font-bold text-slate-700">{t('notes')}</label>
                <textarea value={newLead.notes} onChange={(e) => setNewLead({ ...newLead, notes: e.target.value })} placeholder={t('notesPlaceholder')} rows={3} className="mt-1 w-full p-2.5 text-xs rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#0066FF]" />
              </div>
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                <Button type="button" variant="ghost" onClick={() => setIsAddOpen(false)} className="text-xs rounded-xl cursor-pointer">{tCommon('cancel')}</Button>
                <Button type="submit" disabled={creating} className="bg-[#0066FF] hover:bg-[#0052CC] text-white font-bold text-xs rounded-xl shadow-2xs cursor-pointer">
                  {creating ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : t('btnCreateLead')}
                </Button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-end bg-black/40 backdrop-blur-xs">
          <div className="bg-white w-full max-w-xl h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            {drawerLoading ? (
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-5">
                <div className="flex items-start justify-between border-b border-slate-100 pb-4">
                  <div>
                    <Badge variant="info" className="text-[10px] font-bold uppercase">{t('leadDetails')}</Badge>
                    <h2 className="text-lg font-extrabold text-[#16212B] mt-1">{selected.contactName}</h2>
                    <p className="text-[11px] text-slate-500 font-medium mt-0.5">
                      {getSourceLabel(selected.source)} · {selected.interestLevel && getInterestLabel(selected.interestLevel)} · {new Date(selected.createdAt).toLocaleDateString(dateLocale)}
                    </p>
                  </div>
                ...
                <div className="space-y-2">
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t('leadDetails')}:</label>
                ...
                <div className="p-4 bg-blue-50/60 rounded-2xl border border-blue-100 space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[11px] font-bold text-slate-500">{t('phone')}</label>
                    ...
                    <div>
                      <label className="text-[11px] font-bold text-slate-500">{t('email')}</label>
                  ...
                  {!staffError && (
                    <div>
                      <label className="text-[11px] font-bold text-slate-500">{t('assignedStaff')}</label>
                  ...
                  <div className="flex items-center gap-2">
                    <Button onClick={saveLead} disabled={busyAction === 'save'} className="bg-[#0066FF] hover:bg-[#0052CC] text-white font-bold text-xs rounded-xl gap-1.5 cursor-pointer">
                      {busyAction === 'save' ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CheckCircle2 className="w-3.5 h-3.5" />} {t('btnSave')}
                    </Button>
                    {selected.phone && (
                ...
                <div className="flex items-center gap-2">
                  {selected.status !== 'converted' && (
                    <Button onClick={() => convert(selected.id)} disabled={busyAction === 'convert'} className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl gap-2 cursor-pointer">
                      {busyAction === 'convert' ? <Loader2 className="w-4 h-4 animate-spin" /> : <UserCheck className="w-4 h-4" />} {t('btnConvert')}
                    </Button>
                  )}
                  {selected.status === 'converted' && (
                    <div className="flex-1 p-3 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-bold flex items-center gap-2">
                      <CheckCircle2 className="w-4 h-4" /> {t('convertedToApplicant')}
                    </div>
                  )}
                  {!selected.convertedApplicantId && (
                    !confirmDelete ? (
                      <Button variant="outline" onClick={() => setConfirmDelete(true)} className="text-rose-600 border-rose-200 hover:bg-rose-50 text-xs rounded-xl font-bold cursor-pointer">{t('btnDeleteLead')}</Button>
                    ) : (
                      <Button variant="destructive" onClick={removeLead} disabled={busyAction === 'delete'} className="text-xs rounded-xl font-bold cursor-pointer">
                        {busyAction === 'delete' ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <AlertTriangle className="w-3.5 h-3.5" />} {tCommon('confirm')}
                      </Button>
                    )
                  )}
                </div>
                ...
                {/* Tags */}
                <div className="space-y-2">
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t('tags')}</label>
                  <div className="flex flex-wrap gap-1.5">
                    {(selected.tags ?? []).map((tTag) => (
                      <button key={tTag} type="button" onClick={() => toggleTag(tTag)} className="cursor-pointer">
                        <Badge variant="info" className="text-[10px] font-bold gap-1">
                          {tTag} <X className="w-3 h-3" />
                        </Badge>
                      </button>
                    ))}
                ...
                {duplicates.length > 0 && (
                  <div className="space-y-2">
                    <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1"><Layers className="w-3.5 h-3.5" /> {t('duplicateNotice')}</label>
                    {duplicates.map((d) => (
                      <div key={d.id} className="p-3 rounded-xl bg-amber-50 border border-amber-200 text-xs space-y-1.5">
                        <div className="flex items-center justify-between">
                          ...
                          <Badge variant="neutral" className="text-[9px] font-bold">{getSourceLabel(d.source)}</Badge>
                        </div>
                        <div className="text-[10px] text-amber-700">{d.phone || d.email} · {getStageLabel(d.status)}</div>
                        <div className="flex gap-2">
                          {!confirmMergeId ? (
                            <button onClick={() => setConfirmMergeId(d.id)} className="text-[10px] font-bold text-[#0066FF] hover:underline cursor-pointer flex items-center gap-1"><Link2 className="w-3 h-3" /> {t('btnMerge')}</button>
                          ) : (
                            <button onClick={() => mergeInto(selected.id, d.id)} disabled={busyAction === 'merge'} className="text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-1 rounded-lg border border-emerald-200 cursor-pointer">
                              {busyAction === 'merge' ? '...' : tCommon('confirm')}
                            </button>
                          )}
                        </div>
                      </div>
                ...
                {/* Notes */}
                <div className="space-y-2">
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t('notes')}</label>
                  <textarea
                ...
                {/* Activity / follow-ups */}
                <div className="space-y-2">
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1"><History className="w-3.5 h-3.5" /> {t('history')}</label>
                  {followUps.map((f) => (
                    <div key={f.id} className="p-3 rounded-xl bg-slate-50 border border-slate-200 text-xs space-y-1">
                      <div className="flex items-center justify-between">
                        <Badge variant={f.type === 'call' ? 'info' : f.type === 'email' ? 'warning' : f.type === 'meeting' ? 'success' : 'neutral'} className="text-[9px] font-bold uppercase">{f.type === 'call' ? t('typeCall') : f.type === 'email' ? t('typeEmail') : f.type === 'meeting' ? t('typeMeeting') : t('typeNote')}</Badge>
                        <span className="text-[10px] text-slate-400 font-semibold">{new Date(f.createdAt).toLocaleString(dateLocale)}</span>
                      </div>
                  ...
                  {followUps.length === 0 && <p className="text-[11px] text-slate-400 font-medium">{t('noLeadsInStage')}</p>}
            ...
            <div className="pt-4 border-t border-slate-100 p-6 flex items-center justify-between">
              <span className="text-[10px] text-slate-400 font-medium">{new Date(selected.createdAt).toLocaleString(dateLocale)}</span>
              <Button variant="ghost" onClick={() => setSelected(null)} className="text-xs rounded-xl cursor-pointer">{tCommon('cancel')}</Button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div className="p-3 rounded-xl bg-white border border-slate-200 space-y-2">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] text-slate-400 font-medium flex items-center gap-1"><Clock className="w-3 h-3" /> {t('btnAddFollowUp')}</span>
      </div>
      <div className="flex gap-2">
        <input
          value={notes}
