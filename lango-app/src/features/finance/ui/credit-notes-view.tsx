...
export function CreditNotesView() {
  ...
  function statusBadge(status: CreditNote['status']) {
    if (status === 'approved') {
      return <Badge className="bg-[#DDF5EC] text-[#17A673] text-[10px] border-none font-bold">{tStatus('approved')}</Badge>;
    }
    if (status === 'rejected') {
      return <Badge className="bg-rose-100 text-rose-600 text-[10px] border-none font-bold">{tStatus('rejected')}</Badge>;
    }
    return <Badge className="bg-amber-100 text-amber-700 text-[10px] border-none font-bold">{tStatus('pending')}</Badge>;
  }
  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-[#16212B] tracking-tight">{t('creditNotesTitle')}</h1>
          <p className="text-xs text-slate-500 mt-1">{t('creditNotesSubtitle', { count: notes.length })}</p>
        </div>
        {can('finance.manage') && (
      ...
      {can('finance.manage') && showForm && (
        <Card className="p-5 bg-white rounded-2xl border border-slate-200/80 shadow-2xs space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 text-xs">
            ...
            <div className="space-y-1">
              <label className="font-bold text-slate-600">{t('amount')} (MAD)</label>
        ...
        <table className="w-full text-start text-xs">
          <thead className="bg-[#F6F9FC] text-[#16212B] font-extrabold border-b border-slate-200/80">
            <tr>
              ...
              <th className="py-3.5 px-4">{tCommon('reason')}</th>
