  ...
  return (
    <div className="space-y-6 max-w-[1800px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-[#16212B] tracking-tight">{t('paymentReceiptsTitle')}</h1>
          <p className="text-xs text-slate-500 mt-1">{t('paymentReceiptsSubtitle', { count: receipts.length })}</p>
        </div>
        ...
        <Card className="bg-white rounded-2xl border border-slate-200/80 shadow-2xs overflow-hidden">
          <div className="p-4 flex items-center gap-2 border-b border-slate-100">
            <div className="relative flex-1 min-w-[180px]">
              ...
              <Input placeholder={t('searchReceiptPlaceholder')} value={search} onChange={e => setSearch(e.target.value)} className="ps-8 h-8 text-[11px] bg-slate-50 rounded-xl border-slate-200" />
            </div>
          </div>
            ...
            <table className="w-full text-[11px]">
              <thead>
                <tr className="text-slate-400 font-bold border-b border-slate-100 bg-slate-50/50">
                  <th className="py-2.5 px-3 text-start">{t('receiptNumberCol')}</th>
                  <th className="py-2.5 px-3 text-start">{t('student')}</th>
                  <th className="py-2.5 px-3 text-end">{t('amount')}</th>
                  <th className="py-2.5 px-3 text-center">{tCommon('date')}</th>
                  <th className="py-2.5 px-3 text-start">{t('receivedByCol')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {!loading && filtered.length === 0 && (
                  <tr><td colSpan={5} className="py-8 text-center text-slate-400">{t('noReceiptsFound')}</td></tr>
                )}
                {filtered.map((r) => (
                  <tr
                    key={r.id}
        ...
        {selected && (
          <Card className="p-5 bg-white rounded-2xl border border-slate-200/80 shadow-2xs space-y-4 sticky top-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[10px] font-bold text-slate-400">{t('receipt')}</p>
                <p className="font-mono font-extrabold text-[#2487B8]">{selected.receiptNumber}</p>
              </div>
              <Badge className="bg-[#DDF5EC] text-[#17A673] border-none font-bold text-[9px]">{t('receiptCollectedBadge')}</Badge>
            </div>

            <div className="space-y-1 text-[11px]">
              <div className="flex justify-between"><span className="text-slate-500">{t('student')}</span><span className="font-bold text-[#16212B]">{selected.studentName}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">{tCommon('date')}</span><span className="font-bold text-[#16212B]">{selected.paymentDate}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">{t('receivedByCol')}</span><span className="font-bold text-[#16212B]">{selected.receivedByName ?? '—'}</span></div>
            </div>

            <div className="space-y-2">
              <p className="text-[11px] font-extrabold text-[#16212B]">{t('invoiceBreakdown')}</p>
              <div className="divide-y divide-slate-100 border-t border-b border-slate-100">
                {selected.allocations.length === 0 && <p className="py-2 text-[10px] text-slate-400">{t('noAllocations')}</p>}
