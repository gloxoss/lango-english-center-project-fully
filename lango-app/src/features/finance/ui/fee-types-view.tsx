...
export function FeeTypesView() {
  ...
  const openEdit = (tFee: FeeType) => {
    setEditing(tFee);
    setForm({
      name: tFee.name,
      description: tFee.description ?? '',
      code: tFee.code ?? '',
      taxable: tFee.taxable,
      refundable: tFee.refundable,
      discountable: tFee.discountable,
      fineable: tFee.fineable,
      revenueAccountId: tFee.revenueAccountId ?? '',
      effectiveFrom: tFee.effectiveFrom,
      effectiveTo: tFee.effectiveTo ?? '',
    });
    setError(null);
    setShowForm(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (json.success) {
        ...
        setError(json.message ?? json.error?.message ?? t('saveFailed'));
      }
    } catch {
  ...
  const handleArchive = async (tFee: FeeType) => {
    if (!window.confirm(t('archiveConfirmPrompt', { name: tFee.name }))) return;
    setSaving(true);
    try {
      const res = await fetch('/api/finance/fee-types', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: tFee.id, isArchived: true }),
      });
      const json = await res.json();
      if (json.success) load();
      else window.alert(json.message ?? t('archiveFailed'));
    } catch {
      window.alert(t('connectionError'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const filtered = types
    .filter(tItem => (showArchived ? tItem.isArchived : !tItem.isArchived))
    .filter(tItem => `${tItem.name} ${tItem.code ?? ''} ${tItem.description ?? ''}`.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="space-y-6 max-w-[1200px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-[#16212B] tracking-tight">{t('feeTypesTitle')}</h1>
          <p className="text-xs text-slate-500 mt-1">{t('feeTypesSubtitle', { count: types.filter(tItem => !tItem.isArchived).length })}</p>
        </div>
        {canManage && (
      ...
      <Card className="p-3 bg-white rounded-2xl border border-slate-200/80 shadow-2xs flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div className="relative w-full sm:w-80">
          ...
          <Input placeholder={t('searchFeeTypePlaceholder')} value={search} onChange={e => setSearch(e.target.value)} className="ps-9 h-9 text-xs rounded-xl bg-slate-50 border-none" />
        </div>
      ...
      {canManage && showForm && (
        <Card className="p-5 bg-white rounded-2xl border border-slate-200/80 shadow-2xs space-y-3">
          <h3 className="text-xs font-extrabold text-[#16212B]">{editing ? t('editFeeTypeTitle') : t('newFeeTypeTitle')}</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
            ...
            <div className="space-y-1">
              <label className="font-bold text-slate-600">{t('codeOptionalLabel')}</label>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            {([
              ['taxable', t('taxableLabel'), form.taxable],
              ['refundable', t('refundableLabel'), form.refundable],
              ['discountable', t('discountableLabel'), form.discountable],
              ['fineable', t('fineableLabel'), form.fineable],
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
            <div className="space-y-1">
              <label className="font-bold text-slate-600">{t('revenueAccountOptionalLabel')}</label>
        ...
        <table className="w-full text-start text-xs">
          <thead className="bg-[#F6F9FC] text-[#16212B] font-extrabold border-b border-slate-200/80">
            <tr>
              ...
              <th className="py-3.5 px-4 text-start">{tCommon('description')}</th>
              <th className="py-3.5 px-4 text-start">{t('rulesCol')}</th>
              <th className="py-3.5 px-4 text-start">{t('revenueAccountCol')}</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {!loading && filtered.length === 0 && (
              <tr><td colSpan={canManage ? 7 : 6} className="py-8 text-center text-slate-400">{t('noFeeTypesFound', { status: showArchived ? t('statusArchived') : t('statusActive') })}</td></tr>
            )}
            {filtered.map(tItem => (
              <tr key={tItem.id} className={`hover:bg-slate-50/80 transition font-medium ${tItem.isArchived ? 'opacity-50' : ''}`}>
                <td className="py-3.5 px-4 font-mono text-[11px] text-[#2487B8]">{tItem.code ?? '—'}</td>
                <td className="py-3.5 px-4 font-bold text-[#16212B]">{tItem.name}</td>
                <td className="py-3.5 px-4 text-slate-500 max-w-[220px] truncate">{tItem.description ?? '—'}</td>
                <td className="py-3.5 px-4">
                  <div className="flex flex-wrap gap-1">
                    <Flag on={tItem.taxable} label={t('taxableBadge')} off={t('notTaxableBadge')} />
                    <Flag on={tItem.refundable} label={t('refundableBadge')} off={t('notRefundableBadge')} />
                    <Flag on={tItem.discountable} label={t('discountableBadge')} off={t('notDiscountableBadge')} />
                    <Flag on={tItem.fineable} label={t('fineableBadge')} off={t('notFineableBadge')} />
                  </div>
                </td>
                <td className="py-3.5 px-4 text-slate-500">{accountLabel(tItem.revenueAccountId)}</td>
                <td className="py-3.5 px-4 text-center">
                  <Badge className={`text-[10px] border-none font-bold ${tItem.isArchived ? 'bg-slate-100 text-slate-500' : 'bg-[#DDF5EC] text-[#17A673]'}`}>
                    {tItem.isArchived ? t('statusArchived') : t('statusActive')}
                  </Badge>
                </td>
                {canManage && (
                  <td className="py-3.5 px-4">
                    <div className="flex items-center justify-end gap-1">
                      {!tItem.isArchived && (
                        <>
                          <button onClick={() => openEdit(tItem)} className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-[#2487B8]" title={t('editFeeTypeBtn')}>
                            <Pencil className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => handleArchive(tItem)} className="p-1.5 rounded-lg text-slate-400 hover:bg-rose-50 hover:text-rose-600" title={t('archiveFeeTypeBtn')}>
