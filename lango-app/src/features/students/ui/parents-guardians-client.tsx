  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: t('activeHouseholds'), value: String(households.length), change: t('registeredGuardians'), icon: Home, color: 'text-[#2487B8]', bg: 'bg-[#DCEBF4]' },
          { label: t('withPhone'), value: String(households.filter(h => h.primaryTutorPhone !== '—').length), change: t('contactProvided'), icon: Users, color: 'text-[#17A673]', bg: 'bg-[#DDF5EC]' },
          { label: t('linkedChildren'), value: String(households.reduce((sum, h) => sum + h.children.length, 0)), change: t('totalRecorded'), icon: Phone, color: 'text-amber-600', bg: 'bg-amber-50' },
          { label: t('incompleteFiles'), value: String(households.filter(h => h.primaryTutorPhone === '—' || h.primaryTutorEmail === '—').length), change: t('missingPhoneEmail'), icon: AlertTriangle, color: 'text-[#E5544B]', bg: 'bg-[#FCE4E2]' },
        ].map((kpi) => (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-[#F6F9FC] text-[#16212B] font-extrabold border-b border-slate-200/80">
                  <tr>
                    <th className="py-3.5 px-4">{t('householdNameHeader')}</th>
                    <th className="py-3.5 px-4">{t('guardiansHeader')}</th>
                    <th className="py-3.5 px-4 text-center">{t('linkedStudentsHeader')}</th>
                    <th className="py-3.5 px-4">{t('relationHeader')}</th>
                    <th className="py-3.5 px-4">{t('contactHeader')}</th>
                    <th className="py-3.5 px-4 text-right">{t('portalAccessHeader')}</th>
                  </tr>
                </thead>
                  ...
                  {filtered.map(hh => (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 text-right">
                        ...
                        >
                          {hh.portalAccess ? t('activePortal') : t('noAccount')}
                        </span>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="w-80 xl:w-96 shrink-0 space-y-3 sticky top-6 self-start max-h-[calc(100vh-3rem)] overflow-y-auto">
            <Card className="bg-white rounded-2xl border border-slate-200/80 shadow-2xs overflow-hidden">
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${selectedHousehold.portalAccess ? 'bg-[#DDF5EC] text-[#17A673]' : 'bg-slate-100 text-slate-500'}`}>
                    {selectedHousehold.portalAccess ? t('portalAccessActive') : t('noAccount')}
                  </span>
                </div>
              ...
              <div className="p-4 space-y-4">
                ...
                <div>
                  <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider mb-2">{t('primaryGuardian')}</p>
                ...
                {/* Enfants liés */}
                <div>
                  <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider mb-2">{t('linkedChildrenCount', { count: selectedHousehold.children.length })}</p>
                ...
                <Link
                  href={`/${currentLocale}/dashboard/students/parents/${selectedHousehold.id}`}
        ...
        <DialogContent className="max-w-md bg-white rounded-2xl p-6">
          ...
          <div className="space-y-3 my-3 text-xs">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="font-bold text-slate-700 block mb-1">{t('preferredLanguage')}</label>
                <select
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="font-bold text-slate-700 block mb-1">{t('firstChildLabel')}</label>
                <Input
                  placeholder={t('firstChildPlaceholder')}
              ...
              <div>
                <label className="font-bold text-slate-700 block mb-1">{t('childClassLabel')}</label>
                <Input
