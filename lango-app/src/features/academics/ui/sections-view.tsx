...
type Section = { id: string; name: string; schoolId: string };

type ModalState =
  | { mode: 'closed' }
  | { mode: 'create' }
  | { mode: 'edit'; section: Section }
  | { mode: 'delete'; section: Section };
...
export function SectionsView({ locale: _locale }: { locale: string }) {
  ...
  const fetchSections = useCallback(async (pg: number) => {
    setLoading(true);
    setError(null);
    try {
      const params = new URLSearchParams({ page: String(pg), pageSize: String(PAGE_SIZE) });
      const res = await fetch(`/api/academics/sections?${params}`);
  ...
  const openEdit = (s: Section) => { setFormName(s.name); setFormError(null); setModal({ mode: 'edit', section: s }); };
  const openDelete = (s: Section) => { setModal({ mode: 'delete', section: s }); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const body = isEdit
        ? { id: (modal as { mode: 'edit'; section: Section }).section.id, name: formName.trim() }
        : { name: formName.trim() };
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/academics/sections?id=${modal.section.id}`, { method: 'DELETE' });
      const json = await res.json();
      if (!res.ok || !json.success) throw new Error(json.message ?? `HTTP ${res.status}`);
      closeModal();
