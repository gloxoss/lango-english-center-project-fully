...
// ponytail: real list over the assessments/assessmentPlans built for grade
// entry - this page used to duplicate that concept with fake data. No
// separate "create evaluation" flow here; that already lives at
// dashboard/academics/grades/entry (pick class+subject -> create assessment).
export function EvaluationsView({ locale }: { locale: string }) {
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Link
          ...
          className="h-9 px-4 rounded-full bg-[#0066FF] hover:bg-[#0052CC] text-white text-xs font-bold flex items-center gap-2"
        >
          + Nouvelle évaluation
        </Link>
      </div>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-[#F6F9FC] text-slate-500 font-semibold border-b border-slate-200/80">
              <tr>
                ...
                <th className="py-3 px-4">Notes saisies</th>
              ...
              {sessions.map(s => (
                <tr key={s.id} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="py-3.5 px-4 whitespace-nowrap">
                    <Badge className={s.gradedCount > 0 ? 'bg-[#D1F5E8] text-[#17A673]' : 'bg-[#FCF0DC] text-[#E8A33D]'}>
                      {s.gradedCount}
                      {' '}
                      élève(s)
                    </Badge>
                  ...
                  <td className="py-3.5 px-4 text-center whitespace-nowrap">
                    <Link href={`/${locale}/dashboard/academics/grades/entry`} className="text-[#0066FF] hover:underline text-[11px] font-bold">
                      Saisir les notes
                    </Link>
                  </td>
                </tr>
