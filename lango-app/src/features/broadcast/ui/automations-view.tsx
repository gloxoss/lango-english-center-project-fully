...
import { api, CHANNEL_BADGE, fmtDate, fmtCount, isAddonNotActivated, type ApiErrorShape } from './broadcast-ui';
...
export function AutomationsView() {
  ...
  const testRun = async (a: Automation) => {
    ...
    if (res.ok) setMsg({ ok: true, text: `${t('btnRunTest')} : ${res.data?.queuedCount ?? 0} ${t('enqueued')}` });
    else setMsg({ ok: false, text: res.error?.message ?? tCommon('error') });
    load();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Button onClick={() => setShowForm((v) => !v)} className="cursor-pointer"><Plus className="me-2 h-4 w-4" /> {t('btnNewAutomation')}</Button>
      </div>
      ...
      {msg && (
        <div className={`flex items-center gap-2 rounded-lg px-3 py-2 text-sm ${msg.ok ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
          {msg.ok ? <Sparkles className="h-4 w-4 shrink-0" /> : <AlertCircle className="h-4 w-4 shrink-0" />} {msg.text}
        </div>
      )}
      ...
      {showForm && (
        <Card className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-2xs">
          ...
          <div className="mt-4 flex gap-2">
            <Button onClick={submit} disabled={saving || !name.trim() || !connectionId || !templateId} className="cursor-pointer">
              {saving ? <Loader2 className="me-2 h-4 w-4 animate-spin" /> : <Sparkles className="me-2 h-4 w-4" />} {t('btnCreate')}
            </Button>
            ...
            <Card key={a.id} className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-2xs">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#D1F5E8] text-[#16212B]"><Sparkles className="h-5 w-5" /></div>
                  <div>
                    <p className="font-semibold text-[#16212B]">{a.name}</p>
                    <p className="text-xs text-slate-500">{getKindLabel(a.kind)} · {t('sendTime')} {a.sendTime}</p>
                  </div>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <Badge className={`border ${CHANNEL_BADGE[a.channel]}`}>{getChannelLabel(a.channel)}</Badge>
                  <Badge className={`border ${a.isActive ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-slate-100 text-slate-500'}`}>{a.isActive ? t('active') : t('inactive')}</Badge>
                  <Button variant="outline" size="sm" onClick={() => toggle(a)} className="cursor-pointer"><Power className="me-1 h-3.5 w-3.5" /> {a.isActive ? t('btnDeactivate') : t('btnActivate')}</Button>
                  <Button variant="outline" size="sm" onClick={() => testRun(a)} className="cursor-pointer"><Play className="me-1 h-3.5 w-3.5" /> {t('btnRunTest')}</Button>
                  <Button variant="ghost" size="sm" onClick={() => toggleRuns(a)} className="cursor-pointer">{expanded[a.id] ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}</Button>
                  <Button variant="ghost" size="sm" className="text-slate-400 hover:text-rose-600 cursor-pointer" onClick={() => del(a)}><Trash2 className="h-3.5 w-3.5" /></Button>
                </div>
              </div>
              {expanded[a.id] && (
                <div className="mt-4 border-t border-slate-100 pt-4">
                  <h3 className="mb-2 text-sm font-semibold text-[#16212B]">{t('runs')}</h3>
                  {(!runs[a.id] || (runs[a.id]?.length ?? 0) === 0) && <p className="text-sm text-slate-400">{t('noRuns')}</p>}
                  {runs[a.id]?.map((r) => (
                    <div key={r.id} className="mb-2 flex flex-wrap items-center gap-3 rounded-xl border border-slate-100 bg-slate-50/60 px-3 py-2 text-sm">
                      <span className="font-medium text-[#16212B]">{fmtDate(r.runDate, locale)}</span>
                      <Badge className={`border ${r.status === 'completed' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-slate-100 text-slate-500'}`}>{r.status === 'completed' ? t('completedStatus') : r.status}</Badge>
                      <span className="text-xs text-slate-500">{t('foundCount')} : <b>{fmtCount(r.createdCount, locale)}</b></span>
                      <span className="text-xs text-slate-500">{t('enqueuedCount')} : <b>{fmtCount(r.queuedCount, locale)}</b></span>
                      <span className="text-xs text-slate-500">{t('skippedCount')} : <b>{fmtCount(r.skippedCount, locale)}</b></span>
                    </div>
                  ))}
                </div>
              )}
