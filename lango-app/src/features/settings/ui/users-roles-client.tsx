...
import {
  ...
  AlertTriangle, Lock, Eye, Mail, Building2, ChevronRight,   RefreshCw, Check, X, ShieldAlert, ArrowUpRight, ArrowRight, Copy, Trash2, Clock,
...
function accessScopeForRole(role: string): string {
  if (role === 'super_admin' || role === 'school_admin' || role === 'accountant') return 'all_classes';
  ...
  async function handleRevokeInvitation(id: string) {
    try {
      ...
      if (res.ok && json.success) {
        ...
        toast.success(t('invitationRevoked') || 'Invitation révoquée avec succès.');
      } else {
        toast.error(json?.error?.message || json?.message || 'Erreur lors de la révocation.');
      }
    } catch (err) {
      console.error('Failed to revoke invitation:', err);
      toast.error('Erreur réseau lors de la révocation.');
    }
  }
    ...
    <div className="max-w-[1600px] mx-auto flex flex-col gap-6 pb-20">
      ...
      <div className="flex items-center gap-2 border-b border-[#E5E7EB] pb-2">
        ...
        >
          ...
          {t('tabInvitations')} {pendingInvitesCount > 0 && (
              ...
              <table className="w-full text-left text-xs">
                <thead className="bg-[#F9FAFB] text-[#6B7280] font-semibold border-b border-[#E5E7EB]">
                  <tr>
                    ...
                    <th className="py-3 px-4">{t('colScope')}</th>
                    <th className="py-3 px-4">{t('colCampus')}</th>
                  ...
                  {filteredUsers.map(u => {
                    ...
                    return (
                      <tr key={u.id} className="hover:bg-[#F9FAFB] transition-colors">
                        ...
                        <td className="py-3 px-4 text-[#6B7280]">{u.schoolName || t('school')}</td>
                        <td className="py-3 px-4 text-[#9CA3AF] text-[11px]">{u.lastLogin || t('neverLoggedIn')}</td>
              ...
              {ACCESS_SCOPES.map(scope => (
                <div key={scope.id} className="bg-white p-4 rounded-2xl border border-[#E5E7EB] flex flex-col justify-between space-y-3">
                  <div>
                    ...
                    <p className="text-xs text-[#6B7280] mt-1">{t(`scopes.${scope.id}.description` as 'scopes.school.description')}</p>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-5 rounded-2xl border border-[#E5E7EB] space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Link href={`/${locale}/dashboard/settings/audit-logs`} className="text-xs text-[#4B6BFB] font-medium hover:underline">
                  {t('seeAll')}
                </Link>
              </div>
                  ...
                  <div key={evt.id} className="flex items-start gap-3 text-xs">
                    ...
                    <div>
                      <p className="text-[#111827] font-medium">
                        <span className="font-semibold">{evt.actorName || t('system')}</span> {t('didAction')}{' '}
                        <span className="font-mono bg-[#F9FAFB] px-1 rounded text-[#4B6BFB]">{evt.action}</span> {t('onEntity', { entity: evt.entityType })}
                      </p>
                      <p className="text-[10px] text-[#9CA3AF] mt-0.5">{evt.timestamp || t('recently')}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="bg-white rounded-2xl border border-[#E5E7EB] overflow-hidden shadow-2xs">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-[#F9FAFB] text-[#6B7280] font-semibold border-b border-[#E5E7EB]">
                  <tr>
                    <th className="py-3 px-4">{t('colInvitedEmail')}</th>
                    <th className="py-3 px-4">{t('colAssignedRole')}</th>
                    <th className="py-3 px-4">{t('colStatus')}</th>
                    <th className="py-3 px-4">{t('colExpiry')}</th>
                    <th className="py-3 px-4">{t('colInviteLink')}</th>
                    ...
                    filteredInvitations.map(inv => {
                      ...
                      let statusBadge = (
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-200">
                          <Clock className="w-3 h-3 mr-1" /> {t('invite.pending')}
                        </span>
                      );
                      if (effectiveStatus === 'accepted') {
                        statusBadge = (
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">
                            <CheckCircle2 className="w-3 h-3 mr-1" /> {t('invite.accepted')}
                          </span>
                        );
                      } else if (effectiveStatus === 'revoked') {
                        statusBadge = (
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-700 border border-slate-200">
                            <X className="w-3 h-3 mr-1" /> {t('invite.revoked')}
                          </span>
                        );
                      } else if (effectiveStatus === 'expired') {
                        statusBadge = (
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-50 text-rose-700 border border-rose-200">
                            <AlertTriangle className="w-3 h-3 mr-1" /> {t('invite.expired')}
                          </span>
                        );
                      }
      ...
      {activeTab === 'matrix' && (
        <div className="bg-white p-6 rounded-2xl border border-[#E5E7EB] space-y-6 shadow-2xs">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#F3F4F6] pb-4">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-[#6B7280] font-medium">{t('selectedRole')}</span>
              <select
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-[#F9FAFB] text-[#6B7280] font-semibold border-b border-[#E5E7EB]">
                <tr>
                  <th className="py-3 px-4">{t('colModuleCapability')}</th>
