  ...
  if (children.length === 0) {
    return (
      <div className="flex items-center gap-2 px-4 py-2.5 text-sm text-slate-500 border border-dashed border-slate-300 rounded-xl bg-slate-50">
        ...
        <span>{tParent('noChildrenLinkedShort')}</span>
      </div>
    );
  }
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-3 px-4 py-2 bg-white border border-slate-200 hover:border-[#0066FF]/50 rounded-xl shadow-sm transition-all text-left cursor-pointer"
      >
        ...
        <div>
          <div className="text-[11px] text-slate-500 font-medium flex items-center gap-1">
            <span>{tParent('selectedStudent')}</span>
      ...
      {isOpen && (
        <div
          role="listbox"
          aria-label={tParent('enrolledChildren', { count: children.length })}
        ...
        >
          <div className="px-3 py-2 text-xs font-semibold text-slate-500 border-b border-slate-100 flex items-center gap-1.5">
            ...
            <span>{tParent('enrolledChildren', { count: children.length })}</span>
          </div>
          {children.map((child) => {
