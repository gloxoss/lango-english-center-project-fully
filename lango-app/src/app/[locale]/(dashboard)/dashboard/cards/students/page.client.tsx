...
import { Users, Search, RefreshCw, IdCard, Loader2 } from 'lucide-react';
  ...
  const load = async () => {
    ...
    try {
      const [sRes, iRes] = await Promise.all([
        fetch('/api/students?page=1&pageSize=200'),
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="p-5 rounded-2xl border border-slate-200/80 bg-white shadow-2xs flex items-center justify-between">
          <div>
            ...
            <h3 className="text-2xl font-extrabold text-[#16212B] mt-1">{students.length}</h3>
          </div>
