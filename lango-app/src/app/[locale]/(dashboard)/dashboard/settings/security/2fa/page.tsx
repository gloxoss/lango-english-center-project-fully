...
import {
  ShieldCheck, ShieldX, QrCode, Key, Eye, EyeOff, Copy,
...
// A very minimal inline QR code display using a data URI approach.
// In production this would use a QR library; here we link to an external
// renderer only for display (no secrets sent externally — the URI only
// contains the already-disclosed otpauth:// URL from the server).
function QrCodeDisplay({ otpauthUri }: { otpauthUri: string }) {
  // Render a simple text fallback for the secret + instructions.
  // For real QR rendering, install 'qrcode.react' and replace this.
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="p-3 bg-slate-50 rounded-xl">
        <p className="text-[10px] text-slate-500 mb-1 font-semibold">Ou entrez le code manuellement :</p>
...
export default function TwoFactorPage() {
  ...
  // Step 2: Verify OTP and get backup codes
  const verifyOtp = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.backupCodes?.length) {
        setBackupCodes(json.backupCodes);
        setStep('codes');
        setOtp('');
      } else if (json.user?.twoFactorEnabled) {
        // Some Better Auth versions don't return backup codes on verify
        setBackupCodes([]);
        setStep('codes');
        setOtp('');
      } else {
      ...
      <Card className="p-5 rounded-2xl border border-slate-200 shadow-xs">
        ...
        {step === 'qr' && (
          <div className="space-y-4">
            <QrCodeDisplay otpauthUri={otpauthUri} />
