...
import { guardVisitCheckOutSchema } from '@/features/guard/models/guard-validation';
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const body = await parseJson(request, guardVisitCheckOutSchema);
    const result = await checkOutVisit(context, id, body);
