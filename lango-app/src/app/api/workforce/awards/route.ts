...
export async function POST(request: Request) {
  try {
    ...
    const [employee] = await db.select({ id: employeeProfiles.id, userId: employeeProfiles.userId, branchId: employeeProfiles.branchId }).from(employeeProfiles).where(and(eq(employeeProfiles.id, body.employeeId), eq(employeeProfiles.tenantId, tenantId))).limit(1);
    assertBranchScope(ctx, employee?.branchId ?? null);
