...
export async function GET(request: Request) {
  try {
    ...
    const missingRows = bankBatchIds.length
      ? await db.select({
        ...
        }).from(salaryPayments).leftJoin(employeeProfiles, and(eq(employeeProfiles.tenantId, tenantId), eq(employeeProfiles.userId, salaryPayments.userId))).where(and(eq(salaryPayments.tenantId, tenantId), inArray(salaryPayments.batchId, bankBatchIds))).groupBy(salaryPayments.batchId)
      : [];
  ...
  try {
    ...
    const batch = await db.transaction(async (tx) => {
      ...
      if (!run || run.status !== 'posted') {
        throw new ApiError(409, 'PAYROLL_RUN_NOT_PAYABLE', 'Seule une paie comptabilisée peut être mise en paiement.');
      }
      // A run stays 'posted' until its batch is reconciled, so a second prepare used to
      // reach the unique index and fail with a vague duplicate error. Only a failed or
      // reversed batch may be followed by a new one.
      const [liveBatch] = await tx.select({ id: salaryPaymentBatches.id, status: salaryPaymentBatches.status }).from(salaryPaymentBatches).where(and(eq(salaryPaymentBatches.tenantId, tenantId), eq(salaryPaymentBatches.runId, body.runId), notInArray(salaryPaymentBatches.status, ['failed', 'reversed']))).limit(1);
      if (liveBatch) {
        throw new ApiError(409, 'PAYMENT_BATCH_ACTIVE', 'Un lot de paiement est déjà en cours pour cette paie. Marquez-le en échec avant d’en préparer un autre.');
      }
      ...
      if (!lines.length) {
        throw new ApiError(409, 'PAYROLL_NO_LINES', 'Aucune ligne de paie à payer.');
      }
      ...
      if (!created) {
        throw new ApiError(500, 'PAYMENT_BATCH_FAILED', 'Impossible de créer le lot.');
      }
