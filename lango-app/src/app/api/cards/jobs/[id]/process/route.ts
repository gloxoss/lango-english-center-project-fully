...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [job] = await db.select().from(documentGenerationJobs).where(and(eq(documentGenerationJobs.tenantId, tenantId), eq(documentGenerationJobs.id, id))).limit(1);
    if (!job) {
      throw new ApiError(404, 'NOT_FOUND', 'Lot introuvable.');
    }
    ...
    const [template] = await db.select().from(documentTemplates).where(and(eq(documentTemplates.tenantId, tenantId), eq(documentTemplates.id, version.templateId))).limit(1);
    if (!template) {
      throw new ApiError(404, 'NOT_FOUND', 'Modèle introuvable.');
    }

    const items = await db.select().from(documentGenerationItems).where(and(
    ...
    if (items.length === 0) {
      ...
      await db.update(documentGenerationJobs)
        ...
        .where(and(eq(documentGenerationJobs.tenantId, tenantId), eq(documentGenerationJobs.id, job.id)));
    ...
    for (const item of items) {
      try {
        ...
        const [existing] = await db.select({ id: issuedDocuments.id }).from(issuedDocuments).where(and(
        ...
        if (existing) {
          await db.update(documentGenerationItems)
            ...
            .where(and(eq(documentGenerationItems.tenantId, tenantId), eq(documentGenerationItems.id, item.id)));
          successDelta += 1;
          continue;
        }
    ...
    const [pendingRow] = await db.select({ value: count() }).from(documentGenerationItems).where(and(
      eq(documentGenerationItems.tenantId, tenantId),
      eq(documentGenerationItems.jobId, job.id),
      eq(documentGenerationItems.status, 'pending'),
    ));
