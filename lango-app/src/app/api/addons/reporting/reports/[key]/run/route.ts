...
export async function POST(
  ...
  try {
    ...
    checkRateLimit(`report-run:${tenantId}:${context.userId}`, 20, 60 * 60 * 1000);

    const { parameters, format } = await parseJson(request, runSchema);
    const runId = await RunEngine.queueRun(tenantId, key, parameters ?? {}, format ?? 'csv', context.userId);

    recordAudit(context, 'export', 'report_run', runId, { reportKey: key, format: format ?? 'csv' });
