...
import { bulkCommitAllocations } from '@/features/hostel/services/allocation-service';
...
export async function POST(request: Request) {
  try {
    ...
    const body = await parseJson(request, bulkCommitSchema);
    const allocations = await bulkCommitAllocations(tenantId, context.userId, body.rows);
    recordAudit(context, 'create', 'hostel_allocation', 'bulk', { count: allocations.length });
    return NextResponse.json({ success: true, data: allocations }, { status: 201 });
  } catch (error) {
